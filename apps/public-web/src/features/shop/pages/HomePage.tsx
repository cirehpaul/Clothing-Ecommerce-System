...
import { useCategories } from '@/features/shop/hooks/useProducts'
...
import { useHomepageData } from '@/features/shop/hooks/useStorefront'

// ... (keep the features and testimonials for now as they are not dynamic in the mock API) ...

export default function HomePage() {
  const { data: categories } = useCategories()
  const { data: homepageData, isLoading } = useHomepageData()

  const heroBanner = homepageData?.banners?.find((b: any) => b.type === 'hero')
  const promoBanner = homepageData?.banners?.find((b: any) => b.type === 'promotional')
    ...
    <div>
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0">
          <img
            src={heroBanner?.imageUrl || 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=1400&q=80'}
            alt="Hero"
        ...
        <div className="container relative z-10">
          ...
          >
            ...
            >
              ✦ {heroBanner?.title || 'New Collection'}
            </motion.span>
            <motion.h1
            ...
            >
              {heroBanner?.title?.split(' ')[0] || 'Wear Your'}<br />
              <span style={{ color: '#E5E5E5' }}>{heroBanner?.title?.split(' ').slice(1).join(' ') || 'Story.'}</span>
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              {heroBanner?.subtitle || 'Premium clothing crafted for modern life.'}
            </motion.p>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div>
                ...
                <h2 className="text-3xl md:text-4xl font-black text-white mt-2 mb-3" style={{ fontFamily: 'var(--font-display)' }}>
                  {promoBanner.title.split(' — ')[0] || promoBanner.title}<br />
                  <span style={{ color: '#FCD34D' }}>{promoBanner.title.split(' — ')[1] || 'Special Offer'}</span>
                </h2>
              ...
              <Link
                to={promoBanner.linkUrl || '/products?sale=true'}
                className="btn btn-lg flex-shrink-0"
                style={{ background: 'white', color: '#111111' }}
              >
                {promoBanner.buttonText || 'Shop Sale'} <ArrowRight size={16} />
              </Link>
              {/* Decorative circles */}
      ...
      {sections.map((section: any, index: number) => {
        if (section.type === 'categories') return null; // We render categories separately above

        const linkHref = 
          section.type === 'featured' ? '/products?featured=true' :
          section.type === 'new_arrivals' ? '/products?sort=newest' :
          '/products?sort=popular';
          
        return (
          <div key={section.id} style={{ background: index % 2 === 0 ? 'var(--bg-secondary)' : 'var(--bg)' }}>
            <ProductRow
              label="Collection"
              title={section.title}
